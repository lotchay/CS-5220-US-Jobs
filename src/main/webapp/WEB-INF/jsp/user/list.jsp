						...
						<tr>
							...
							<td><security:authorize access="hasRole('ROLE_ADMIN')">
									...
									<a data-userid="${user.id}" class="btn btn-success edit"
										role="button" href="edit?id=${user.id}"><i
										class="fa fa-pencil-square-o" aria-hidden="true"></i>&nbsp;&nbsp;Edit</a>
									<c:if test="${user.enabled}">
										<!-- This is refreshing the page b/c there is an href on the anchor tag that
										redirects the page to the servlet url
										
										1) Make href="javascript:void(0)" 
										...
										<a data-userid="${user.id}" class="btn btn-danger disable"
											role="button" href="disable?id=${user.id}"><i
											class="fa fa-level-down" aria-hidden="true"></i>&nbsp;&nbsp;Disable</a>
											
										<!-- 1)  This is one option that works. Note that we make href="javascript:void(0)"
										in order to make the anchor tag clickable, since there is no role="button" attribute -->
								<%-- 		<a data-userid="${user.id}" class="btn btn-danger disable"
											href="javascript:void(0)"><i
											class="fa fa-level-down" aria-hidden="true"></i>&nbsp;&nbsp;Disable</a> --%>
											
										<!-- 2) This is another option. We remove the href completely, but make the anchor tag <a> 
										clickable by adding the attribute role="button" -->
										<%-- <a data-userid="${user.id}" class="btn btn-danger disable"
											role="button"><i
											class="fa fa-level-down" aria-hidden="true"></i>&nbsp;&nbsp;Disable</a> --%>
									</c:if>
									<c:if test="${not user.enabled}">
										<!--   Do the same fixes here as above -->
										<a data-userid="${user.id}" class="btn btn-warning enable"
											role="button" href="enable?id=${user.id}"><i
